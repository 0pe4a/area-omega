...
if not isfolder("Neutral\\Themes") then
	pcall(makefolder, "Neutral")
	pcall(makefolder, "Neutral\\Themes")
end
...
for i, v in next, themes do
	if i ~= "selected" then
		writefile("Neutral\\Themes\\" .. i .. ".json", jsonEncodeTheme(v))
	end
end
...
function library.new(gameName)
	local lib = setmetatable({
		_name = gameName,
		_gui = create("ScreenGui", { Name = "Neutral", ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, {
			create("Frame", { Name = "main", BackgroundColor3 = "theme.mainBackground", ClipsDescendants = true, Position = UDim2.new(0, 100, 0, 100), Size = UDim2.new(0, 535, 0, 380) }, {
				...
				create("Frame", { Name = "top", BackgroundColor3 = "theme.topBackground", Size = UDim2.new(1, 0, 0, 34), ZIndex = 2 }, {
					create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 34, 0, 0), Size = UDim2.new(1, -68, 1, 0), Text = "Neutral | " .. gameName, TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
				...
				create("TextLabel", { Name = "site", AnchorPoint =  Vector2.new(0.5, 1), BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0.5, 0, 1, 0), Size = UDim2.new(1, -16, 0, 26), Text = "funny gui lol", TextColor3 = "theme.textForeground", TextSize = 12, TextXAlignment = Enum.TextXAlignment.Right }),
...
function library:GetConfigs()
	local configs = {}
	if isfolder("Neutral\\Configs\\" .. self._name) then
		for i, v in next, listfiles("Neutral\\Configs\\" .. self._name) do
...
function library:LoadConfig(name)
	local filePath = "Neutral\\Configs\\" .. self._name .. "\\" .. name .. ".json"
	if not (isfolder("Neutral\\Configs\\" .. self._name) and isfile(filePath)) then
		return
	end
...
function library:SaveConfig(name)
	if not isfolder("Neutral\\Configs\\" .. self._name) then
		pcall(makefolder, "Neutral")
		pcall(makefolder, "Neutral\\Configs")
		pcall(makefolder, "Neutral\\Configs\\" .. self._name)
	end
	local config = {}
	for _, tab in next, self._tabs do
		for _, panel in next, tab._panels do
	...
	writefile("Neutral\\Configs\\" .. self._name .. "\\" .. name .. ".json", httpService:JSONEncode(config))
end
...
function library:GetThemes()
	...
	for i, v in next, listfiles("Neutral\\Themes") do
		local split = select(-1, unpack(v:split("\\")))
		if split:find(".json") then
...
function library:LoadTheme(name)
	local filePath = "Neutral\\Themes\\" .. name .. ".json"
	if not (isfolder("Neutral\\Themes") and isfile(filePath)) then
		print("noes")
		return
	end
